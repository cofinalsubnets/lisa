...
static Inline u1 tblp(ob _) { return
  homp(_) && (typ) GF(_) == &tbl_typ; }
static Inline u1 strp(ob _) { return
  homp(_) && (typ) GF(_) == &str_typ; }
static Inline  u1 twop(ob _) { return
  homp(_) && (typ) GF(_) == &two_typ; }
static Inline u1 symp(ob _) { return
...
static U copy(la, size_t);
static void copy_to(la, size_t, ob*);
static ob *fresh_pool(size_t n) { return malloc(n * sizeof(ob)); }
...
bool please(struct V *v, size_t req) {
...
static void *bump(la v, size_t n) {
  void *x = v->hp;
  return v->hp += n, x; }

static void *cells(la v, size_t n) { return
...
static intptr_t hash(la, ob);

static str str_ini(void *_, size_t len) {
  str s = _; return
...
static two two_ini(u0 *_, ob a, ob b) {
  two w = _; return
    w->act = act,
    w->typ = &two_typ,
    w->a = a, w->b = b,
...
#define VM0(_) _(gc) _(xok) _(yield)
...
static bool neql(la v, ob x, ob y) { return false; }
static bool eql(la v, ob a, ob b) { return a == b ||
  (!nump(a|b) && G(a) == act &&
   ((typ) GF(a))->equi(v, a, b)); }
static NoInline void report(la, const char*, ...);
static void show_error(la v, enum status s) {
  switch (s) {
...
static void transmit(la, FILE*, ob);

static NoInline void report_call(la v, mo ip, frame fp) {
  putc('(', stderr);
...
// FIXME actnn't distinguish between OOM and parse error
...
static vm ap_str, ap_nop, ap_two, ap_tbl;
...
static tbl tbl_ini(u0 *_, U len, U cap, struct tbl_e **tab) {
  tbl t = _; return
    t->act = act,
    t->typ = &tbl_typ,
    t->len = len,
...
static tbl tbl_set(la v, tbl t, ob k, ob x) { return
  t = tbl_set_s(v, t, k, x),
  t ? tbl_grow(v, t) : 0; }
...
static u0 tx_str(struct V *v, FILE *o, ob _) {
  str s = (str) _;
  size_t len = s->len;
...
static u1 eq_str(struct V *v, ob x, ob y) {
...
//
static sym sym_ini(u0 *_, str nom, U code) {
  sym y = _; return
    y->act = act,
    y->typ = &sym_typ,
    y->nom = nom,
...
static vm setclo, genclo0, genclo1;
...
static NoInline enum status la_go(struct V*);
...
static NoInline enum status
la_call(la v, mo f, size_t n) { return
...
int main(int ac, char **av) {
  ...
  for (;;) switch (getopt(ac, av, "hi_")) {
    ...
    case -1:
      ...
      return show_error(&v, s), la_fin(&v), s; } }
