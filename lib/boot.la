...
; macros.
 ...
 fuse (:
  ...
  ((pushi i) j p) (?
   (= i-push (peek p)) (pokef i (seek p 1))
   (poke j p))
 ...
 (, (ev prelu) (ev co-data) (ev co-source)
    ((ev 'ev) prelu) ((ev 'ev) co-data) ((ev 'ev) co-source)))

; " drop privileges "
; leave these in for now while we work on the compiler
(tdel _ns
 'inliners 'fuse 'pure 'abelians 'monoids
